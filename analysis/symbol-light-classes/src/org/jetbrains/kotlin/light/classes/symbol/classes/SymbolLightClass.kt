...
internal open class SymbolLightClass(classOrObject: KtClassOrObject) : SymbolLightClassForClassOrObject(classOrObject) {
    init {
    ...
    private fun addFieldsForEnumEntries(result: MutableList<KtLightField>, classOrObjectSymbol: KtNamedClassOrObjectSymbol) {
        if (isEnum) {
            classOrObjectSymbol.getDeclaredMemberScope().getCallableSymbols()
                ...
                .mapNotNullTo(result) {
                    val enumEntry = it.psi<KtEnumEntry>()
                    val name = enumEntry.name ?: return@mapNotNullTo null
                    SymbolLightFieldForEnumEntry(it.psi(), name, this@SymbolLightClass, null)
                }
        }
    }
    ...
    override fun copy(): SymbolLightClass = SymbolLightClass(classOrObject)
}
